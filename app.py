...
# Secure File Uploads with Random Folder Names
...
# Enhanced Face Comparison Logic
def compare_photos(reference_image_path, live_image_path):
    ...
    except Exception as e:  # Handle all potential DeepFace exceptions
        print(f"Error during face comparison: {e}")
        return False

# Face Authentication API Route
...
def compare():
    ...
    # Create a temporary directory for secure storage
    upload_folder = os.path.join(app.config['UPLOAD_FOLDER'], generate_random_folder())
    os.makedirs(upload_folder)  # Create the temporary directory
    ...
    try:
        ...
        os.rmdir(upload_folder)  # Remove the temporary directory
